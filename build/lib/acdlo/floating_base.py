...
f_FK = sm.lambdify((q,p,s,d), pickle.load(open(Path(__file__).parent / "sympy_fcns/fb/fk", "rb")), "mpmath")
def eval_fk(q, p_vals, s, d): 
    return np.array(f_FK(q, p_vals, s, d).apply(mp.re).tolist(), dtype=float)

f_FKA = sm.lambdify((q,p,s,d), pickle.load(open(Path(__file__).parent / "sympy_fcns/fb/fka", "rb")), "mpmath")
def eval_fka(q, p_vals, s, d): 
    return np.array(f_FKA(q, p_vals, s, d).apply(mp.re).tolist(), dtype=float)

f_J_end_wrt_base = sm.lambdify((q,p), pickle.load(open(Path(__file__).parent / "sympy_fcns/fb/J_end_wrt_base", "rb")), "mpmath")
...
# f_G = sm.lambdify((q,p), pickle.load(open(Path(__file__).parent / "sympy_fcns/fb/G", "rb")), "mpmath")
# def eval_G(q, p_vals): 
#     return np.array(f_G(q, p_vals).apply(mp.re).tolist(), dtype=float)

# f_Gv = sm.lambdify((q,gamma,p), pickle.load(open(Path(__file__).parent / "sympy_fcns/fb/Gv", "rb")), "mpmath")
# def eval_Gv(q, gamma, p_vals): 
#     return np.array(f_Gv(q, gamma, p_vals).apply(mp.re).tolist(), dtype=float)

# f_B = sm.lambdify((q,p), pickle.load(open(Path(__file__).parent / "sympy_fcns/fb/B", "rb")), "mpmath")
# def eval_B(q, p_vals): 
#     return np.array(f_B(q, p_vals).apply(mp.re).tolist(), dtype=float)

# f_C = sm.lambdify((q,dq,p), pickle.load(open(Path(__file__).parent / "sympy_fcns/fb/C", "rb")), "mpmath")
